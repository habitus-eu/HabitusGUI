...
GGIRshiny = function(accdir, outputdir, configfile=c(), sleepdiary=c()) {
  if (is.null(sleepdiary)) sleepdiary = c()
  if (length(sleepdiary) > 0) {
    GGIR::g.shell.GGIR(datadir=accdir, outputdir=outputdir, configfile = configfile, loglocation=sleepdiary)
  } else { 
    GGIR::g.shell.GGIR(datadir=accdir, outputdir=outputdir, configfile = configfile)
  }
}