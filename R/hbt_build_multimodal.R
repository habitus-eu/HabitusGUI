...
hbt_build_multimodal <- function(data = NULL,
  ...
  triptype = func = mot = variable = value = start_trip = end_trip = NULL
  ...
  if (!exists("multimodal_fields") & !is.null(config_file)) {
    multimodal_fields <- hbt_read_config(config_file) %>%
  ...
  if (!is.null(config_file)) {
    trajectory_locations <- hbt_read_config(config_file) %>%
